...
export const bot = async ({ sock, msg, fullArgs }: CommandHandlerParams) => {
    if (!fullArgs) {
        return sock.sendMessage(msg.key.remoteJid!, { text: 'Hello! How can I help you today?' });
    }
    try {
        await sock.sendPresenceUpdate('composing', msg.key.remoteJid!);
        const response = await generateText(`User asked: ${fullArgs}\nBot response:`);
        await sock.sendMessage(msg.key.remoteJid!, { text: response }, { quoted: msg });
    } catch (error) {
        console.error("Bot command error:", error);
        await sock.sendMessage(msg.key.remoteJid!, { text: 'Sorry, I had trouble processing that.' });
    } finally {
        await sock.sendPresenceUpdate('paused', msg.key.remoteJid!);
    }
};
bot.description = "Chat with the AI bot (alias for .ai).";
bot.category = "ai_chat";
bot.usage = ".bot <your message>";

export const chatbot = bot; // Alias
chatbot.description = "Chat with the AI bot (alias for .ai).";
chatbot.category = "ai_chat";
chatbot.usage = ".chatbot <your message>";
